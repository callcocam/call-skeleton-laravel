...
{
    public function __construct(protected PlanogramSaleRepositoryContract $saleRepository) {}
    ...
    public function summary(string $productId): JsonResponse
    {
        ...
        $product = $productModelClass::query()->findOrFail($productId);

        // Busca vendas agregadas do produto
        $salesSummary = $this->saleRepository->salesQuery()
        ...
        $salesByMonth = $this->saleRepository->salesQuery()
            ->where('product_id', $product->id)
        ...
        $topStores = $this->saleRepository->salesQuery()
            ->where('product_id', $product->id)
