...
class GondolaController extends ResourceController
{
    ...
    {
        $categoryModelClass = $this->categoryModelClass();
        ...
        $gondola->load(['planogram.category']);
        // Note: planogram.client removed - use $gondola->planogram->client instead (custom getter)

        // Obter IDs de categorias (hierarquia)
        $categoryIds = [];
        if ($categoryId) {
            $category = $categoryModelClass::query()->find($categoryId);
            if ($category) {
    ...
    public function updateImages($gondola)
    {
        ...
        $categoryIds = $this->getCategoryIdsForPlanogram($planogram);
        $eans = $productModelClass::query()
            ->when($planogram->client_id, fn ($q) => $q->where('client_id', $planogram->client_id))
        ...
        $database = $client->database ?? config('plannogram.tenant_database');
        if (! $database) {
     ...
     */
    protected function getCategoryIdsForPlanogram(Planogram $planogram): array
    {
        $categoryModelClass = $this->categoryModelClass();
    ...
    protected function canCreateGondola($model): bool
    {
        ...
        return $model->flowConfig()->with('steps')->exists() && $model->flowConfig->steps->isNotEmpty();
    }
