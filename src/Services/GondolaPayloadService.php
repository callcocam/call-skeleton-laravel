...
namespace Callcocam\LaravelRaptorPlanogram\Services;
...
{
    public function __construct(protected PlanogramStoreRepositoryContract $storeRepository) {}
        ...
        if ($planogram = $gondola->planogram) {
            ...
            $gondolas = $planogram->gondolasStarted
                ->map(function ($relatedGondola) {
                    ...
                    if ($execution->current_responsible_id === auth()->id()) {
                        return [
            ...
            $recordData['planogram']['gondolas'] = array_filter($gondolas); // Remove gondolas vazias (sem permissão)
            ...
            if ($planogram->store_id) {
                $store = $this->storeRepository->findMapDataById($planogram->store_id);
                if ($store !== null) {
                    $recordData['planogram']['store'] = $store;
                }
            }
        }
