...
namespace Callcocam\LaravelRaptorPlanogram\Services\Analysis;

use Callcocam\LaravelRaptorPlanogram\Contracts\PlanogramProductContract;
use Callcocam\LaravelRaptorPlanogram\Contracts\PlanogramSaleRepositoryContract;
use Callcocam\LaravelRaptorPlanogram\Contracts\PlanogramSalesSummaryRepositoryContract;
...
class AbcAnalysisService
{
    public function __construct(
        protected PlanogramSaleRepositoryContract $saleRepository,
        protected PlanogramSalesSummaryRepositoryContract $salesSummaryRepository,
    ) {}
     ...
     *
     * @param  object  $category  Categoria e seus pais serão incluídos
    ...
    public function analyzeByCategory(
        ...
        $productIds = $productModelClass::query()->whereIn('category_id', $allCategoryIds)
            ->pluck('id')
            ->toArray();
        ...
        $codigosErp = $productModelClass::query()
    ...
    public function analyzeByEans(
        ...
        $productModelClass = $this->productModelClass();
        $products = $productModelClass::query()->whereIn('ean', $eans)->get();
    ...
    public function analyzeByCodigoErp(
        ...
        if (! isset($filters['client_id']) || empty($filters['client_id'])) {
            Log::error('ABC Analysis - client_id é obrigatório para análise por codigo_erp');
            throw new \InvalidArgumentException('client_id é obrigatório para análise ABC');
        }
        ...
        $productModelClass = $this->productModelClass();
        $products = $productModelClass::query()->with(['category'])->whereIn('id', $productIds)->get()->keyBy('id');
    ...
    private function getLastSalesForProducts(array $productIds): Collection
    {
        ...
        return $this->saleRepository->salesQuery()
            ->withoutGlobalScopes()
    ...
    private function getSalesQueryByCodigoErp(array $codigosErp, array $filters): Builder
    {
        ...
        $query = $this->saleRepository->salesQuery()
            ->withoutGlobalScopes()
            ->join('products', 'products.codigo_erp', '=', 'sales.codigo_erp')
            ->select([
                'products.id as product_id',
    ...
    private function getMonthlySummariesQueryByCodigoErp(array $codigosErp, array $filters): Builder
    {
        ...
        $query = $this->salesSummaryRepository->summaryQuery()
            ->withoutGlobalScopes()
    ...
    private function calculatePercentagesAndClassification(
        ...
        foreach ($products as $product) {
            ...
            $productModel = $productsCache?->get($product['product_id'])
                ?? $productModelClass::query()->with(['category'])->find($product['product_id']);
