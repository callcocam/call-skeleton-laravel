...
class IAPlanogramService
{
    public function __construct(
        ...
        protected PlanogramTenantConnectionResolverContract $tenantConnectionResolver,
    ) {}
    ...
    {
        $currentClientId = config('plannogram.current_client_id');

        if (! $currentClientId) {
            throw new \RuntimeException('Cliente atual não definido (current_client_id)');
        }

        $this->tenantConnectionResolver->setupClientConnectionFromConfig();

        if (! config('plannogram.tenant_database')) {
            throw new \RuntimeException("Cliente não encontrado: {$currentClientId}");
        }

        Log::info('🔗 Conexão tenant configurada', [
            'client_id' => $currentClientId,
            'database' => config('plannogram.tenant_database'),
        ]);
    }
    ...
    protected function selectProducts(IAGenerateConfigDTO $config, Gondola $gondola): array
    {
        ...
        foreach (['A', 'B', 'C'] as $class) {
            ...
            $productsInClass = $productModelClass::on('tenant')
        ...
        if ($selectedProducts->count() < $totalProducts) {
            ...
            $additionalProducts = $productModelClass::on('tenant')
                ->whereIn('category_id', $categoryIds)
    ...
    protected function inferCategoryFromGondola(Gondola $gondola): string
    {
        ...
        $rootCategory = $categoryModelClass::on('tenant')
    ...
    protected function buildCategoryHierarchy(array $categoryIds): array
    {
        ...
        $categories = $categoryModelClass::on('tenant')
        ...
        DB::connection('tenant')->transaction(function () use ($gondola, $result) {
            ...
            foreach ($result->shelves as $shelfData) {
                ...
                foreach ($shelfData['products'] as $productData) {
                    $productModelClass = $this->productModelClass();
                    $product = $productModelClass::on('tenant')->find($productData['product_id']);
